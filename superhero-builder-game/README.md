...
1. **Describe your character** (`characterConcept.ts`): text or voice
   ...
   pick from.
2. **Set traits** (`characterTraits.ts`): choose gender (girl/boy) and add
   one or more powers/energies — description, effect, and the condition
   that turns the power on. The roster is capped (default **5** characters
   per creator, shown at the top of the screen; configurable for later
   upgrade tiers).
...
7. **See it all in one place** (`creatorDashboard.ts`): an aggregated,
   embeddable summary (characters, tools, habitats, cards, custom games,
   active listings) meant to be shown as a tab inside the same dashboard
   app used by the other Mani World products — including a flag for
   whether the creator has paid-tier access to a 3D viewer for rotating
   their characters/tools (real 3D rendering is out of scope here; only the
   access flag is modeled).
8. **Voice input** (`voiceInput.ts`): any typed step can instead be a voice
   note (`pending -> recorded -> approved`), for kids who can't type yet.
9. **Parental safety** (`parentSafety.ts`): explicit parental consent AND a
   passed moderation review are both required before anything is listed in
   the marketplace or sent to physical print/shipping — there is no way to
   skip this gate.

## Scope note

Real AI image generation, 3D-figurine rendering/printing, physical card
printing/shipping, and voice/audio capture all require external services.
This project only models the registries, state machines, and data contracts
around that pipeline — it does not generate images, render 3D models,
print/ship anything, or record audio itself.
