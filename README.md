...
| Admin API | `src/server/` | Express app (`createApp()`) exposing `/api/credentials`, `/api/markets`, `/api/presentation`, and `/api/pages-websites` CRUD/workflow endpoints, protected by an `x-api-key` header (`MW_ADMIN_API_KEY`). `GET /api/i18n` is public and returns the EN/FA/AR translation dictionary; `/public/page/:slug` serves published pages and accepts submissions. |
