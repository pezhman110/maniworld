...
| Pages & Websites | `src/modules/pagesWebsites.ts` | Target-based website, landing page, campaign page and conversion-builder module with 15 templates, block library, brand kits, EN/AR/FA language modes with RTL/LTR public rendering, smart forms, destination routing, target/project/CRM/deal-room connection fields, quality scoring, analytics, A/B variants, referrals, file/voice import records, approval workflow, publishing statuses, version history, and `/public/page/:slug` submission handling. |
| Investor Acquisition | `src/modules/investorAcquisition.ts` | Stage 1/5 investor acquisition engine with the first 20 requested tools as executable backend capabilities: intent detection, persona classification, consent/source-proof compliance gates, lead scoring, priority routing, compliant playbook generation, and metrics. Unsafe requests such as fake/purchased accounts, anti-detection, scraped contacts, private-group monitoring, and face-recognition matching are blocked and replaced with official-API/opt-in/public-business-contact alternatives. Exposed at `/api/investor-acquisition` and dashboard tab "Investor Acquisition". |
| Consent Conversion Accelerator | `src/modules/consentConversionAccelerator.ts` | Compliant account-to-contact output engine: accepts social/account records, blocks scraped/guessed/purchased/fake sources, deduplicates accounts and contacts before spend, scores quality, keeps public-business contacts in permission-first status until opt-in proof exists, and returns final JSON/CSV output at `/api/consent-conversion/final-output`. |
| Admin API | `src/server/` | Express app (`createApp()`) exposing `/api/credentials`, `/api/markets`, `/api/presentation`, and `/api/pages-websites` CRUD/workflow endpoints, protected by an `x-api-key` header (`MW_ADMIN_API_KEY`). The app is hardened by default with no `X-Powered-By` header, `nosniff`/frame/referrer/permissions headers, `Cache-Control: no-store` on `/api/*`, blocked `TRACE` requests, a bounded JSON body size, and JSON parse/size error responses. `GET /api/i18n` is public and returns the EN/FA/AR translation dictionary; `/public/page/:slug` serves published pages and accepts submissions. |
| Admin dashboard | `public/dashboard/` | Static, dependency-free HTML/JS admin UI (served at `/dashboard`) to add/test/remove every integration connection, manage markets, configure presentation campaigns, and manage the new "Pages & Websites" builder. Includes an EN/FA/AR language switcher that sets text direction (`dir="rtl"` for fa/ar). |
